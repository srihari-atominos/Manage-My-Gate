...
* Before finalizing any task, you must silently verify:
  ...
  * The implementation is production-ready.# Global Project Rules (AI Agent Directives)
...
* **Service-Level Isolation:** * The backend must maintain its own `backend/.env` file for secure secrets (DB credentials, JWT secrets, port configurations).
