  ...
  useEffect(() => {
    ...
    if (!sharedSocket) {
      ...
      sharedSocket.on('connect', () => {
        ...
        if (userId) {
          sharedSocket?.emit('join_room', `user:${userId}`);
        }

        if (userRole) {
          sharedSocket?.emit('join_room', `role:${userRole}`);
          sharedSocket?.emit('join_room', `role:${userRole.toLowerCase()}`);
        }

        if (orgId) {
          sharedSocket?.emit('join_room', `org:${orgId}`);
          if (userRole) {
            sharedSocket?.emit('join_room', `org:${orgId}:role:${userRole.toLowerCase()}`);
            sharedSocket?.emit('join_room', `org:${orgId}:role:${userRole}`);
          }
        }
      });
