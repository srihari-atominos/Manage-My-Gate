...
import { QrCode, ShieldAlert, CheckCircle2, Clock, AlertCircle, DoorOpen } from 'lucide-react-native';
...
export function selectDisplayablePass(
  ...
  const resPasses = passes.filter(
    (p) => !p.reservationId || p.reservationId === reservation._id
  );
  ...
  const checkedInPass = resPasses.find(
...
export function ResidentAccessPassCard({
  ...
  const isCheckedOut =
    ...
    Boolean(activePass?.checkedOutAt);

  const isCheckedIn =
    reservation.accessStatus === 'CHECKED_IN' ||
    Boolean(activePass?.checkedInAt && !isCheckedOut);
  ...
  if (isRevoked) {
    return (
      <View
        testID={testID}
        className={`bg-card p-5 rounded-3xl border border-destructive/40 items-center gap-3 ${className}`}
      >
        ...
        <Text variant="muted" className="text-center text-xs text-muted-foreground max-w-xs">
  ...
  if (isCheckedOut) {
    return (
      <View
        testID={testID}
        className={`bg-card p-5 rounded-3xl border border-border items-center gap-3 ${className}`}
      >
    ...
    return (
      ...
      >
        ...
        <View className="items-center gap-1">
          <Text className="font-bold text-base text-foreground">Digital Access Pass</Text>
          <StatusBadge
            label={reservation.accessStatus || 'NOT_APPLICABLE'}
            variant="neutral"
          />
        </View>
        <Text variant="muted" className="text-center text-xs text-muted-foreground max-w-xs">
  ...
  if (isCheckedIn) {
    return (
      <View
        testID={testID}
        className={`bg-card p-5 rounded-3xl border border-primary/30 items-center gap-3 ${className}`}
      >
        ...
        <View className="items-center gap-1">
          <Text className="font-bold text-base text-foreground">Digital Access Pass</Text>
          <StatusBadge label="CHECKED_IN" variant="info" pulseDot />
        </View>
        <Text variant="muted" className="text-center text-xs text-muted-foreground max-w-xs">
  ...
  if (canRenderQr) {
    return (
      <View
        testID={testID}
        className={`bg-card p-5 rounded-3xl border border-border items-center gap-3 shadow-xs ${className}`}
      >
        ...
        <View className="flex-row items-center gap-2 mt-1">
          <StatusBadge
            label={reservation.accessStatus === 'PASS_GENERATED' ? 'PASS_GENERATED' : activePass.status || 'ACTIVE'}
            variant="success"
            pulseDot
          />
        </View>
        ...
        <View className="items-center gap-0.5 mt-1">
          ...
          {(activePass as any).gateId ? (
            <Text variant="muted" className="text-[11px] text-center text-muted-foreground">
  ...
  return (
    ...
    >
      ...
      <Text variant="muted" className="text-center text-xs text-muted-foreground max-w-xs">
        Digital QR presentation credential is not available for this pass. Please present your booking confirmation at the gate.
      </Text>

      {/* Legitimate server metadata presentation */}
      <View className="w-full bg-secondary/50 rounded-2xl p-3 gap-1 mt-1">
        {formattedValidFrom && formattedValidUntil ? (
