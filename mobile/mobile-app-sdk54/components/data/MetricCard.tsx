...
export const MetricCard = ({
  ...
  return (
    <View className={cn('rounded-2xl border border-border bg-card p-4 shadow-xs', className)}>
      <Text className="text-xs font-semibold font-sans text-muted-foreground uppercase tracking-wider mb-2">
        {title}
      </Text>
      ...
      <View className="flex-row items-end justify-between">
        ...
        {trend !== undefined && (
          <View className="flex-row items-center mb-1">
            {getTrendIcon()}
            <Text className={cn('text-xs font-bold font-sans', getTrendColor())}>
              {Math.abs(trend)}%
            </Text>
          </View>
        )}
