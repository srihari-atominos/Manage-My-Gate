 ...
 * Matches the Web Frontend `react-qr-code` output for value={code}.
 */
function createStandardQRMatrix(text: string): boolean[][] {
  const value = (text || '849201').trim();
  const N = 21;
...
export const VisitorQRCode: React.FC<VisitorQRCodeProps> = ({
  ...
  const matrix = useMemo(() => createStandardQRMatrix(code), [code]);
  ...
  return (
    <View className="items-center justify-center p-4 bg-card rounded-2xl border border-border gap-3">
      ...
      <View className="bg-white p-3 rounded-2xl border border-border items-center justify-center shadow-md">
