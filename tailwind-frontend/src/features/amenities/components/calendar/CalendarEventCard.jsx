...
const CalendarEventCard = memo(({ event, onClick, isCompact = false }) => {
  ...
  const getColorClasses = (status) => {
    switch(status) {
      case 'approved':
      case 'confirmed': return 'bg-success bg-opacity-10 border-success text-success';
      case 'pending': return 'bg-warning bg-opacity-10 border-warning text-warning-dark';
      case 'cancelled':
      case 'rejected': return 'bg-danger bg-opacity-10 border-danger text-danger';
      case 'checked_in': return 'bg-info bg-opacity-10 border-info text-info-dark';
      default: return 'bg-secondary bg-opacity-10 border-secondary text-secondary';
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="small text-truncate mb-2" style={{ opacity: 0.85 }}>{event.subtitle}</div>
