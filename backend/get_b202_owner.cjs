...
async function main() {
  ...
  if (!unit) {
    console.log('Unit B-202 not found in the database.');
  } else {
    console.log('Unit Found:', unit.unitNumber, unit.blockOrBuilding);
    ...
    if (unit.ownerId) {
      ...
      if (owner) {
        ...
        console.log('Owner user document not found in "users" collection.');
      }
    }
