...
async function testMC() {
  try {
    ...
    // Check both lowercase and camelCase just in case
    const mcIntegration = await IntegrationHub.findOne({ provider: { $in: ['messagecentral', 'messageCentral'] }, status: 'connected' });
    if (!mcIntegration) {
    ...
    // Try passing authToken DIRECTLY without base64 encoding it!
    const tokenUrl = `https://cpaas.messagecentral.com/auth/v1/authentication/token?customerId=${encodeURIComponent(customerId)}&key=${encodeURIComponent(authToken)}&scope=NEW&country=${encodeURIComponent(countryCode)}`;
