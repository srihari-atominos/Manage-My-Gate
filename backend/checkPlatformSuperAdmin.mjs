...
async function check() {
  ...
  const role = await mongoose.connection.collection('roles').findOne({name: 'Platform Super Admin'});
  if (!role) {
    console.log('Platform Super Admin role not found');
  } else {
    ...
    console.log('Platform Super Admin total perms:', perms.length);
