...
export const useAssessmentForm = ({ onClose, onSuccess, assessment = null }) => {
    ...
    const loadStaticData = async () => {
      ...
      try {
        ...
        if (active && villasRes?.data) {
          ...
          const blockNames = [
            ...new Set(villaList.map((v) => v.block || v.blockOrBuilding).filter(Boolean)),
          ]
  ...
  const handleSelectAll = useCallback(
    (ids) => setSelectedIds((p) => Array.from(new Set([...p, ...ids]))),
    [],
  )
  const handleDeselectAll = useCallback(
    (ids) => setSelectedIds((p) => p.filter((id) => !ids.includes(id))),
    [],
  )
  const handleToggleUType = useCallback(
    (t) => setSelectedUnitTypes((p) => (p.includes(t) ? p.filter((x) => x !== t) : [...p, t])),
    [],
  )
  ...
  const handleTieredRate = useCallback(
    (key, val) => setTieredRates((p) => ({ ...p, [key]: val })),
    [],
  )
  ...
  const errors = useMemo(() => {
    ...
    const isRecurring =
      type === 'RECURRING' || (type === 'CAPITAL_REPAIR' && collectionMethod === 'INSTALLMENT')
    const isOneTime =
