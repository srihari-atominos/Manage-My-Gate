...
const Assignee = () => {
  ...
  const newAssignmentsCount = assignedComplaints.filter(c => c.status === 'Assigned' || c.status === 'Waiting For Acceptance').length;
      ...
      <div className="view-container">
        <div className="view active">
          ...
          {newAssignmentsCount > 0 && (
            <div style={{ backgroundColor: 'var(--primary-pale)', border: '1px solid var(--primary-base)', padding: '12px 16px', borderRadius: '8px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <i className="fa-solid fa-circle-info" style={{ color: 'var(--primary-base)' }}></i>
              <div>
                <strong>Admin allocated work:</strong> You have {newAssignmentsCount} new pending assignment{newAssignmentsCount > 1 ? 's' : ''}.
              </div>
            </div>
          )}
                  ...
                  <table className="ent-table">
                    ...
                    <tbody>
                      {assignedComplaints.length === 0 && !isLoading && (
                        <tr>
                          <td colSpan="6" style={{ textAlign: 'center' }}>No assigned tasks found.</td>
                        </tr>
                      )}
                        ...
                        <tr key={c._id} onClick={() => setSelectedComplaintId(c._id)} style={{ cursor: 'pointer' }}>
                          ...
                          <td>
                            <span className={`badge ${
                              c.priority === 'High' ? 'badge-error' : 
                              c.priority === 'Medium' ? 'badge-warning' : 
                              'badge-info'
                            }`}>{c.priority}</span>
                          </td>
                          <td>{c.category}</td>
                          <td>
                            <span className="status-indicator">
                              <span className={`status-dot ${c.status === 'Resolved' || c.status === 'Closed' ? 'bg-success' : c.status === 'In Progress' ? 'bg-primary' : 'bg-warning'}`}></span>
                              {c.status}
                            </span>
                          </td>
                          <td>
          ...
          <div className="complaint-modal" style={{ display: 'block' }}>
            ...
            <div className="form-group">
              ...
              <textarea 
                ...
                style={{ width: '100%', minHeight: '80px', border: '1px solid var(--border)', borderRadius: '6px', padding: '12px' }}
              ></textarea>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
              <button className="btn btn-ghost" onClick={() => setShowRejectModal(false)}>Cancel</button>
              <button className="btn btn-primary" style={{ background: 'var(--critical)', borderColor: 'var(--critical)', color: '#fff' }} onClick={submitReject}>Confirm Reject</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="complaint-modal-overlay">
          <div className="complaint-modal" style={{ display: 'block' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
              ...
              <button className="btn btn-primary" onClick={submitPause}>Confirm Pause</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCompleteModal && (
        <div className="complaint-modal-overlay">
          <div className="complaint-modal" style={{ display: 'block' }}>
            ...
            {/* Optional Photo Upload UI can be added here if needed */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
              <button className="btn btn-ghost" onClick={() => setShowCompleteModal(false)}>Cancel</button>
              <button className="btn btn-primary" style={{ background: 'var(--success)', borderColor: 'var(--success)' }} onClick={submitComplete}>Complete Work</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="complaint-modal-overlay">
          <div className="complaint-modal" style={{ display: 'block' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
              ...
              <button className="btn btn-primary" onClick={submitUpload}>Upload Photos</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="complaint-modal-overlay">
          <div className="complaint-modal" style={{ display: 'block' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
              ...
              <button className="btn btn-primary" onClick={submitNotes}>Add Notes</button>
            </div>
          </div>
        </div>
      )}
