...
// Thunks
export const fetchActivePolls = createAsyncThunk('poll/fetchActivePolls', async (params, { rejectWithValue }) => {
  try {
...
export const fetchClosedPolls = createAsyncThunk('poll/fetchClosedPolls', async (params, { rejectWithValue }) => {
  try {
...
export const fetchMyPolls = createAsyncThunk('poll/fetchMyPolls', async (params, { rejectWithValue }) => {
  try {
...
export const createPoll = createAsyncThunk('poll/createPoll', async (pollData, { rejectWithValue }) => {
  try {
...
export const voteOnPoll = createAsyncThunk('poll/voteOnPoll', async ({ id, optionIndex }, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(voteOnPoll.pending, (state, action) => {
      ...
      lists.forEach(list => {
        ...
        if (index !== -1) {
          ...
          // Decrement old option if they are changing their vote
    ...
    builder.addCase(voteOnPoll.fulfilled, (state, action) => {
      const poll = { 
        ...action.payload.data, 
        hasVoted: true, 
        votedOptionIndex: action.meta.arg.optionIndex 
      };
      updatePollInList(state.activePolls.data, poll);
      updatePollInList(state.myPolls.data, poll);
      updatePollInList(state.closedPolls.data, poll);
