...
export const LiveEntriesTable = ({ liveEntries, onCheckOutSuccess }) => {
  ...
  const filteredEntries = liveEntries.filter(
    (entry) =>
      ...
      (entry.idProofNumber &&
        entry.idProofNumber.toLowerCase().includes(searchQuery.toLowerCase())),
  )
  ...
  return (
    <div
      className="card invite-form-card"
      style={{ minHeight: '430px', display: 'flex', flexDirection: 'column' }}
    >
      <div style={{ flex: 1 }}>
        <div className="live-entries-card-header">
          <h3 style={{ fontSize: '18px', margin: 0 }}>
            ...
            ></i>{' '}
            Active Visitors Inside ({liveEntries.length})
          </h3>
        ...
        {currentEntries.length === 0 ? (
          <div className="live-entries-empty">
            ...
            <span style={{ fontSize: '14px', fontWeight: '600' }}>
              No active visitors inside community.
            </span>
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>
              All entry log check-ins have checked-out.
            </span>
          </div>
        ) : (
