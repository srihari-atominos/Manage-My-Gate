...
export const WalkInApprovalList = ({ walkins, setWalkins, onApprove, onDeny, logs = [] }) => {
  const handleApprove = (id) => {
    if (onApprove) {
      ...
      setWalkins((prev) =>
        prev.map((item) =>
          item.id === id || item._id === id ? { ...item, status: 'APPROVED' } : item,
        ),
      )
  ...
  const handleDeny = (id) => {
    if (onDeny) {
      ...
      setWalkins((prev) =>
        prev.map((item) =>
          item.id === id || item._id === id ? { ...item, status: 'DENIED' } : item,
        ),
      )
  ...
  const formatLogTime = (time) => {
    ...
    if (
      typeof time === 'string' &&
      (time.includes('now') || time.includes('ago') || time.includes('AM') || time.includes('PM'))
    ) {
      return time
    }
          ...
          {pendingItems.length === 0 ? (
            <div className="empty-state-container">
              ...
              <span className="empty-state-text-sub">
                No visitor is currently waiting at the gate.
              </span>
            </div>
          ) : (
                  ...
                  <div key={itemId} className="card-hover pending-item-card">
                    <div className="d-flex align-items-center gap-3">
                      <div className="pending-item-avatar-wrapper">
                        <img src={photoUrl} alt={visitorName} className="pending-item-avatar" />
                        <span className="pending-item-badge-online" />
                      </div>
                      <div>
                        <div className="item-header-row">
                          <h4 className="item-title">{visitorName}</h4>
                          <span className="item-id-badge">{itemId}</span>
                        </div>
                        <div className="item-detail">
                          <strong>Company:</strong> {company} | <strong>Purpose:</strong> {purpose}
                        </div>
                        <div className="item-meta">
                          <i className="fa-solid fa-car-side me-1"></i> Vehicle: {vehicle} &bull;{' '}
                          <i className="fa-solid fa-user-shield ms-2 me-1"></i> Guard: {guardName}
                        </div>
                      </div>
                    </div>
            ...
            <>
              <div className="history-items-list">
                {currentHistoryLogs.map((item) => {
                  ...
                  const type = (
                    item.type ||
                    item.passId?.passType ||
                    (item.entryType === 'WALK_IN' ? 'WALK_IN' : 'GUEST')
                  ).toUpperCase()
