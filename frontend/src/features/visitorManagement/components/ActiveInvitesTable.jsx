...
export const ActiveInvitesTable = ({
  ...
  const filteredPasses = (passes || []).filter((pass) => {
    ...
    const passMethod =
      pass.method ||
      (pass.passType === 'CAB' || pass.passType === 'DELIVERY'
        ? 'cab_delivery'
        : pass.passType === 'SERVICE'
          ? 'service'
          : pass.passType === 'GUEST' && pass.usageLimit?.maxUses > 5
            ? 'group'
            : 'guest')
              ...
              <tbody>
                {currentPasses.map((pass) => {
                  ...
                  if (pass.validity?.startDate && pass.validity?.endDate) {
                    const start = new Date(pass.validity.startDate).toLocaleDateString('en-US', {
                      day: '2-digit',
                      month: 'short',
                    })
                    const end = new Date(pass.validity.endDate).toLocaleDateString('en-US', {
                      day: '2-digit',
                      month: 'short',
                    })
                      ...
                      <td className="ps-4">
                        ...
                        <div className="item-header-row mt-1">
                          <span
                            ...
                            title={
                              pass.isIdProofPass
                                ? `Pass ID: ${displayPassCode} (ID-Proof Pass)`
                                : `Pass ID: ${displayPassCode} | Click to view QR Code`
                            }
                          >
                        ...
                        <div className="table-actions-container">
                          {pass.isIdProofPass ? (
                            ...
                            >
                              <CIcon
                                icon={cilQrCode}
                                size="sm"
                                style={{ color: 'var(--text-light, #94A3B8)' }}
                              />
                            </button>
                          ) : (
                            <button
                            ...
                            >
                              <CIcon
                                icon={cilQrCode}
                                size="sm"
                                style={{ color: 'var(--primary, #0084FF)' }}
                              />
                            </button>
                          )}
                          <button
                          ...
                          >
                            <CIcon
                              icon={cilCopy}
                              size="sm"
                              style={{ color: 'var(--slate-600, #475569)' }}
                            />
                          </button>
                          {isActive ? (
                            <button
