...
export const useGuardVisitorManagement = () => {
      ...
      : [
          {
            id: 1,
            number: 'Villa 101',
            resident: 'Johnathan Parker',
            status: 'Occupied',
            phone: '+971 50 441 9022',
          },
          {
            id: 2,
            number: 'Villa 102',
  ...
  useEffect(() => {
    if (activeOrgId) {
      ...
      Promise.all([
        apiClient.get('/villas?limit=1000').catch((err) => {
          console.error('Failed to load villas in guard hook:', err)
          return { data: { data: [] } }
        }),
        apiClient.get('/users?limit=1000').catch((err) => {
        ...
        dispatch(
          getPasses({
            orgId: activeOrgId,
            params: { page: 1, limit: 100, statuses: 'PENDING,ACTIVE,EXPIRED' },
          }),
        ).catch((err) => {
          console.error('Failed to load passes in guard hook:', err)
          return null
        }),
        dispatch(getActiveVisitors(activeOrgId)).catch((err) => {
  ...
  const liveEntries =
    reduxActiveVisitors && reduxActiveVisitors.length > 0
      ? reduxActiveVisitors.map((log) => {
