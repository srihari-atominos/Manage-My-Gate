  ...
  return (
    <CCard className="h-100 card-hover border-0 shadow-sm overflow-hidden">
        ...
        <div className="position-absolute top-0 end-0 p-3 d-flex flex-column gap-2 align-items-end">
          ...
          {amenity.maxBookingsPerUserPerSlot && (
            <span className="badge bg-dark shadow-sm">
              Your Booked Slots: {amenity.userBookedSlotsCount || 0} /{' '}
              {amenity.maxBookingsPerUserPerSlot}
            </span>
          )}
        </div>
      ...
      <CCardBody className="d-flex flex-column p-4">
        <div className="text-uppercase text-muted small fw-bold mb-1">{amenity.type}</div>
        <h4 className="mb-2 text-truncate" title={amenity.name}>
          {amenity.name}
        </h4>
        ...
        >
          {amenity.description ||
            'Enjoy our premium facilities tailored for your comfort and recreation.'}
        </p>
        ...
        <div className="d-flex align-items-center text-muted small mb-4 gap-3">
          ...
          <div>
            <i className="fa-solid fa-users me-2 text-primary"></i>Up to {amenity.capacity || 'N/A'}
          </div>
        </div>
