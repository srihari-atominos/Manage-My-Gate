...
const TimelineStep = ({ icon, iconBg, title, subtitle, isLast = false, isDenied = false }) => (
  <div className="d-flex gap-3">
    <div className="d-flex flex-column align-items-center">
      <div
        ...
        style={{
          width: '36px',
          height: '36px',
          background: isDenied ? '#fee2e2' : iconBg || '#e0f2fe',
          zIndex: 1,
        }}
      >
        <i
          className={`fa-solid ${icon} small`}
          style={{ color: isDenied ? '#ef4444' : '#0284c7' }}
        ></i>
      </div>
      {!isLast && (
        <div
    ...
    <div className="pb-4">
      <p className="small fw-semibold mb-0" style={{ color: isDenied ? '#ef4444' : '#0f172a' }}>
        {title}
      </p>
...
const DetailRow = ({ label, value, mono = false }) => (
  <li className="list-group-item px-0 py-2 d-flex justify-content-between align-items-start border-0 border-bottom">
    <span className="text-muted small fw-semibold" style={{ minWidth: '120px' }}>
      {label}
    </span>
    <span
      className={`text-end fw-semibold small ${mono ? 'font-monospace' : ''}`}
...
const SectionHeader = ({ icon, title }) => (
  <div className="d-flex align-items-center gap-2 mb-3 mt-4">
    ...
    <h6 className="small fw-bold mb-0" style={{ color: '#0f172a' }}>
      {title}
    </h6>
  </div>
)
    ...
    >
      ...
      >
        <div className="d-flex align-items-center gap-3">
          <div className="rounded-circle bg-body bg-opacity-25 p-2">
            <i
              className={`fa-solid ${isSuccess ? 'fa-shield-check' : 'fa-shield-exclamation'} text-white fa-lg`}
            ></i>
          </div>
          <div>
      ...
      <div className="offcanvas-body p-4" style={{ overflowY: 'auto' }}>
        ...
        >
          {log.residentPhoto ? (
            ...
            <div
              ...
              style={{ width: '56px', height: '56px', background: '#e2e8f0' }}
            >
        ...
        <ul className="list-group list-group-flush">
          ...
          <DetailRow
            label="Scan Time"
            value={scanTime?.toLocaleString('en-US', {
              weekday: 'short',
              month: 'short',
              day: 'numeric',
              year: 'numeric',
          ...
          />
          {log.entryTime && (
            <DetailRow label="Entry Time" value={new Date(log.entryTime).toLocaleTimeString()} />
          )}
          {log.exitTime && (
            <DetailRow label="Exit Time" value={new Date(log.exitTime).toLocaleTimeString()} />
          )}
        </ul>
