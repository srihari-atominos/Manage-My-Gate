...
const AmenityGrid = memo(({ amenities, canManage, canUpdate, canDelete, onEdit, onToggleStatus, onViewDetails }) => {
  if (!amenities || amenities.length === 0) {
    return <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>No amenities match your criteria.</div>;
  }

  return (
    <div className="dashboard-grid" id="amenity-master-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
      {amenities.map((item) => {
        const imageUrl = (item.images && item.images.length > 0) ? item.images[0] : 'https://images.unsplash.com/photo-1519710164239-da123dc03ef4?auto=format&fit=crop&w=800&q=80';
        const rate = item.pricing?.baseRate ?? item.ratePerHour ?? 0;
        const pricingLabel = item.pricing?.pricingType === 'daily' ? '/day' : item.pricing?.pricingType === 'session' ? '/session' : '/hr';

        return (
          <div key={item._id} className="card card-hover amenity-item-card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div 
              style={{ 
                height: '180px', 
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <div style={{ color: 'var(--text-muted)', marginBottom: '16px', lineHeight: 1.8, flex: 1 }} className="fw-medium small">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <i className="fa-solid fa-users" style={{ color: 'var(--primary)', width: '14px', textAlign: 'center' }}></i>
                  <span>Capacity: {item.capacity || 'N/A'}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <i className="fa-solid fa-indian-rupee-sign" style={{ color: 'var(--primary)', width: '14px', textAlign: 'center' }}></i>
                  <span>₹{rate.toLocaleString()}{pricingLabel}</span>
                </div>
              </div>
