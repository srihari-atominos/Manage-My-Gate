...
const AdminMaintenanceView = () => {
  ...
  const handleSave = async (amenityId, maintenanceData) => {
    try {
      if (selectedTask) {
        await dispatch(
          editMaintenance({ amenityId, maintenanceId: selectedTask._id, data: maintenanceData }),
        ).unwrap()
      } else {
  ...
  const handleDelete = async (task) => {
    if (window.confirm('Are you sure you want to delete this maintenance task?')) {
      try {
        await dispatch(
          removeMaintenance({ amenityId: task.amenityId, maintenanceId: task._id }),
        ).unwrap()
        dispatch(fetchMaintenanceList())
      } catch (err) {
          ...
          >
            <div>
              ...
              <p style={{ color: 'var(--text-muted)', margin: 0 }} className="fw-medium">
                Track upkeep tasks and block amenities while work is in progress.
              </p>
            </div>
            <button
                  ...
                  {loading && maintenanceList.length === 0 ? (
                    ...
                    paginatedTasks.map((task) => (
                      <tr key={task._id}>
                        ...
                        <td>
                          <span
                            className={`badge badge-${task.status === 'completed' ? 'success' : task.status === 'in_progress' ? 'warning' : 'primary'}`}
                            style={{
