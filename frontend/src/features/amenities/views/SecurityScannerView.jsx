...
const SecurityScannerView = () => {
  const { scanResult, loading, error, recentScans, handleScan, handleManualEntry, resetScanner } =
    useSecurityScanner()
    ...
    <div className="amenities-module-wrapper amenity-os-theme">
      ...
      <div className="view-container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h1 style={{ margin: 0 }} className="fs-1">
              Security Scanner
            </h1>
                    ...
                    <div className="last-scan-details">
                      ...
                      <div className="bg-body p-3 rounded border mb-3">
                        ...
                        <div className="d-flex justify-content-between mb-2">
                          ...
                          <span
                            className={`badge ${recentScans[0].scanType === 'Exit' ? 'bg-info' : 'bg-success'}`}
                          >
                            {recentScans[0].scanType}
                          </span>
                        </div>
                        <div className="d-flex justify-content-between mb-2">
                          <span className="text-muted small">Scan Time</span>
                          <span className="fw-medium small">
                            {new Date(recentScans[0].scanTime).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                      ...
                      {recentScans && recentScans.length > 0 ? (
                        recentScans.map((scan) => (
                          <tr key={scan._id}>
                            ...
                            <td>
                              <span
                                className={`badge ${scan.scanType === 'Exit' ? 'badge-info' : 'badge-success'}`}
                              >
                                {scan.scanType}
                              </span>
                            </td>
                            <td>
                              <span className="badge badge-success text-white">
                                <i className="fa-solid fa-check me-1"></i> {scan.result}
                              </span>
                            </td>
