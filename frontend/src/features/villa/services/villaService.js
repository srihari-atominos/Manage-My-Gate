 ...
 */
export const fetchVillas = async ({ page = 1, limit = 12, search = '', blockOrBuilding = '', status = '', type = '' } = {}) => {
...
export const assignExistingUser = async (villaId, userId, residencyType) => {
  const response = await apiClient.post(`/villas/${villaId}/assign-resident`, { userId, residencyType });
  return response.data;
};
...
export const updateResidencyType = async (villaId, userId, residencyType) => {
  const response = await apiClient.patch(`/villas/${villaId}/residents/${userId}/type`, { residencyType });
  return response.data;
};
